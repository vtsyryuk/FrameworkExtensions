...
namespace System.Collections.Concurrent
{
	...
	{
		private readonly ConcurrentDictionary<TRequestId, Tuple<TaskCompletionSource<TResponse>, Timer>> _pendingRequests;
		...
		public RequestHandler(TimeSpan timeout, IEqualityComparer<TRequestId> comparer)
		{
			...
			_pendingRequests = new ConcurrentDictionary<TRequestId, Tuple<TaskCompletionSource<TResponse>, Timer>>(comparer);
			_timeout = timeout;
		}
		...
		public int CancelPendingRequests()
		{
			...
			{
				var completion = TryRemoveRequestCompletion(requestId);
